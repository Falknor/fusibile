                      ...
                      )
{
  ...
  // Note: required for cuda mem alloc
  ...
  sprintf(output_folder,
          "%s/consistencyCheck-%04d%02d%02d-%02d%02d%02d/",
          results_folder.c_str(),
          pTime->tm_year + 1900,
          pTime->tm_mon + 1,
          pTime->tm_mday,
          pTime->tm_hour,
  ...
  for (int j = 0; j <camParams.cameras.size(); ++j)
  {
    camParams.viewSelectionSubset.push_back(j);
  }
