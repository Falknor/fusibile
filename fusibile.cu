 ...
 * Output: either depth or disparity value
 */
__device__ FORCEINLINE float disparityDepthConversion_cu2(const float &f,
...
__global__ void fusibile(GlobalState &gs, int ref_camera)
{
  ...
  for (int i = 0; i < camParams.viewSelectionSubsetNumber; i++)
  {
        ...
        tmp_pt.y < rows)
    {
      ...
      const float
          depth_disp =
          disparityDepthConversion_cu2(camParams.cameras[ref_camera].f,
                                       camParams.cameras[ref_camera],
                                       camParams.cameras[idxCurr],
                                       depth);
      const float
          tmp_normal_and_depth_disp =
          disparityDepthConversion_cu2(camParams.cameras[ref_camera].f,
                                       camParams.cameras[ref_camera],
                                       camParams.cameras[idxCurr],
  ...
  float averaging_value = (float) number_consistent + 1.0f;
  consistent_X = consistent_X / averaging_value;
  consistent_normal = consistent_normal / averaging_value;
  consistent_texture =
      {consistent_texture.x / averaging_value,
       consistent_texture.y / averaging_value,
       consistent_texture.z / averaging_value};
  ...
  {
    if (!gs.params->remove_black_background
        || (consistent_texture.x > 15
            && consistent_texture.y > 15
            && consistent_texture.z > 15)) // hardcoded for middlebury TODO FIX
    {
...
template<typename T>
  void fusibile_cu(GlobalState &gs, PointCloudList &pc_list, int num_views)
  {
    ...
    printf("Grid size initrand is grid: %d-%d block: %d-%d\n",
           grid_size_initrand.x,
           grid_size_initrand.y,
           block_size_initrand.x,
